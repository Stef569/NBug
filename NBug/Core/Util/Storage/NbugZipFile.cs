namespace NBug.Core.Util.Storage
{
  ...
  /// <summary>
  /// Represents a NBUG zip file. This zip file can contain:
  /// An exception stored as an XML file
  /// A report XML stored as an XML file
  /// A Minidump file
  /// Additional files
  /// </summary>
  internal class NbugZipFile : IDisposable
  {
    ...
    /// </summary>
    /// <param name="zipFileName">The path of the zip file</param>
    ...
    public void AddException(SerializableException serializableException)
    {
      using (var stream = new MemoryStream())
      {
        ...
        // We need to rewind to the beginning of the stream
        // before adding the stream to the zip file!
        stream.Position = 0;
        _zipfile.AddStream(ZipStorer.Compression.Deflate, StoredItemFile.Exception, stream, DateTime.UtcNow,
          string.Empty);
      }
    }
    ...
    public void AddReport(Report report)
    {
      using (var stream = new MemoryStream())
      {
        ...
        // before adding the stream to the zip file!
        stream.Position = 0;
        _zipfile.AddStream(ZipStorer.Compression.Deflate, StoredItemFile.Report, stream, DateTime.UtcNow,
          string.Empty);
      }
    }

    /// <summary>
    /// Store the minidump into this zip file
    /// </summary>
